...
local function worker(user_args)
    ...
    local menu_items = {
        --{ name = 'Cancel', icon_name = '', command = function() end},
    ...
    for _, item in ipairs(menu_items) do
        ...
        row:buttons(awful.util.table.join(
            ...
            awful.button({}, "3", function()
                popup.visible = not popup.visible end)
        ))
    ...
    function logout_menu_widget.toggle()
        --popup.visible = not popup.visible
        if popup.visible then
            popup.visible = not popup.visible
        else
            awful.placement.top_left(popup, { margins = { top = 40, left = 10}, parent = awful.screen.focused() })
            popup.visible = true
        end
    end
